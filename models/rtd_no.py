...
class RTD(nn.Module):
    ...
    def __init__(self,
        ...
        if self.feature_recon:  
            ...
            self.feature_align = MLP(hidden_dim, hidden_dim, 2048, 2)
    ...
    def forward(self, samples: NestedTensor, patches: torch.Tensor):
        ...
        if self.feature_recon:
            ...
            if self.aux_loss:
                # out['aux_outputs'] = self._set_aux_loss_up(outputs_class, outputs_coord,outputs_iou)
                out['aux_outputs'] = self._set_aux_loss_up(outputs_class, outputs_coord, outputs_iou, outputs_feature, patch_feature_gt)
    ...
    def _set_aux_loss(self, outputs_class, outputs_coord, outputs_iou):
        ...
        return [{
            ...
            'pred_iou': c
                             ...
                             outputs_iou[:-1])]
    @torch.jit.unused
    def _set_aux_loss_up(self, outputs_class, outputs_coord, outputs_iou, outputs_feature, backbone_output):
        ...
        # return [{'pred_logits': a, 'pred_boxes': b, 'pred_iou': d}
        #         for a, b, d in zip(outputs_class[:-1], outputs_coord[:-1],  outputs_iou[:-1])]
        return [{'pred_logits': a, 'pred_boxes': b, 'pred_feature': c, 'pred_iou': d,'gt_feature': backbone_output}
