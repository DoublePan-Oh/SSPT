...
class PositionEmbeddingSine(nn.Module):
    ...
    def _get_encoding_table(self, tensor):
        ...
        table = np.array([
            ...
            for position in range(tensor.shape[1])
        ])
    ...
    def forward(self, locations: NestedTensor):
        position_table =  self._get_encoding_table(locations.tensors)
        device = locations.tensors.device
        result = position_table[:locations.tensors.shape[1]].clone().detach(
        ).repeat(locations.tensors.shape[0], 1, 1)
