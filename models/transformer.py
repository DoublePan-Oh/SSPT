...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed, decoder_mask=None):
        ...
        src = src.permute(2, 0, 1) # torch.Size([17, 32, 512])
        ...
        # print("mask:",mask)
        # print(mask.shape)
        # print("decoder_mask.shape:",decoder_mask)
        tgt = torch.zeros_like(query_embed)
        # memory = self.encoder(src, src_key_padding_mask=mask, pos=pos_embed)

        src = torch.cat([src, pos_embed], dim=2)
        # print("src_cat.shape:",src.shape)
        ...
        # print(hs)
        return hs.transpose(1, 2), memory.permute(1, 2, 0).contiguous().view(bs, c, t)
