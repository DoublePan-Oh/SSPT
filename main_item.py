...
from util.scheduler import WarmupMultiStepLR
...
def get_args_parser():
    ...
    parser.add_argument('--num_patches',
                        default=4,
                        type=int,
                        help='Number of random copped segment for a  video')
    ...
    # parser.add_argument('--dataset_file', default='activityNet')
    # parser.add_argument('--dataset_file', default='HACS')
    # parser.add_argument('--dataset_file', default='ucf101')

    parser.add_argument('--window_size', default=100, type=int)
    parser.add_argument('--interval', default=5, type=int)
    parser.add_argument('--gt_size', default=100, type=int)
    # parser.add_argument('--feature_path',
    #                     default='data/anet_1.3/tsp_features',
    #                     type=str)
    # parser.add_argument('--feature_path',
    #                     default='/root/panpan/code/RTD-Action-ANet/data/anet_features_i3d_feature_25fps_resize-step16',
    #                     type=str)
    # parser.add_argument('--feature_path',
    #                     default='/root/panpan/code/up-detr-master/data/hacs_segments_features/all',
    #                     type=str)
    parser.add_argument('--feature_path',
                        default='/root/panpan/code/RTD-Action/data/I3D_features',
                        type=str)    
    # parser.add_argument('--feature_path',
    #                     default='/root/panpan/code/RTD-Action/data/ucf101/feat_ucf_2048',
    #                     type=str)               
    parser.add_argument('--tem_path', default='data/TEM_scores', type=str)
    parser.add_argument('--annotation_path',
                        default='/root/panpan/code/RTD-Action-master/datasets/thumos14_anno_action.json',
                        type=str)
    # parser.add_argument('--annotation_path',
    #                 default='data/anet_1.3/annotations/anet1.3_tsp_filtered.json',
    #                 type=str)
    # parser.add_argument('--annotation_path',
    #                 default='/root/panpan/code/RTD-Action-ANet/datasets/anet_anno_action.json',
    #                 type=str)
    # parser.add_argument('--annotation_path',
    #                 default='/root/panpan/code/up-detr-master/data/HACS_segments_v1.1.1.json',
    #                 type=str)
    # parser.add_argument('--annotation_path',
    #                     default='/root/panpan/code/RTD-Action/data/ucf101/video_meta_frame.json',
    #                     type=str)
    ...
    parser.add_argument('--output_dir',
                        default='outputs/Thumos/20220626_label0_single_loadbest_frameend/',
    ...
    parser.add_argument('--pretrain', default='',
                    help='initialized from the pre-training model')
    return parser
...
def main(args):
    ...
    if args.pretrain:
        ...
        for k in list(state_dict.keys()):
            ...
            if 'input_proj_boundary' in k:
                del state_dict[k]
            if 'position_embedding' in k:
                del state_dict[k]
            # if 'input_proj.weight' in k:
            #     state_dict[k] = state_dict[k].squeeze(2)
        # # exit()
    ...
    # ax7_train = fig1.add_subplot(237)

    axs_train = {
        'loss_ce': ax1_train,
        'loss_bbox': ax2_train,
        'loss_giou': ax3_train,
        'loss_feature': ax4_train,
        'class_error': ax5_train,
        'loss_iou': ax6_train
        # 'loss_feature': ax7_train
    }
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for key, value in train_loss_dict.items():
            if key in [
                    ...
                    'class_error', 'loss_iou', 'loss_feature'
            ]:
                try:
        ...
        if (float(train_stats['loss']) < min_loss): 
            min_loss = float(train_stats['loss'])
            min_epoch = epoch
            # with (checkpoint_dir / 'log.json').open('a') as f:
            #     f.write(json.dumps(log_stats) + '\n')
            checkpoint_path = output_dir / 'checkpoint_min_loss.pth'
            utils.save_on_master(
                {
                    'model': model_without_ddp.state_dict(),
                    'optimizer': optimizer.state_dict(),
