...
class VideoRecord:
    def __init__(self, vid, locations, num_frames, gt, fps, args, patches, s_e_scores):
        self.id = vid
...
class ThumosDetection(torch.utils.data.Dataset):
    def __init__(self, feature_folder, tem_folder, anno_file, split, args):
        ...
        for vid in video_list:
            # if self.split in vid:
                ...
                end_scores_list = []
                gt_frame = []
                for frame in range(0, num_frames, self.interval):
                    frames.append(frame)
                    start_scores = random.uniform(0,1)
                    start_scores_list.append(start_scores)
                    end_scores = random.uniform(0,1)
                    end_scores_list.append(end_scores)
                    if start_scores > 0.6 or end_scores > 0.6:
                        gt_frame.append(frame)
                # print(gt_frame)
                frames = np.expand_dims(frames,1)
                start_scores = np.expand_dims(start_scores_list,1)
                end_scores = np.expand_dims(end_scores_list,1)
                ...
                while len(length) < self.num_patches: # 10 is num_patch
                    ...
                    start = random.choice(gt_frame)
                    end = random.choice(gt_frame)
                    if end <= start or (end - start < 2 * self.interval):
                        continue
                    ...
                    T_end =  int(end // 8 - 7 )
                    if T_start < 0 or T_end > vid_feature.shape[0] or (T_end <= T_start ) or (pesudo_num_length > self.window_size * self.interval):
                        continue
                ...
                if seq_len <= self.window_size:
                    ...
                    self.video_list.append(
                        VideoRecord(vid, locations, num_frames, gt, 
                                    fps, args, patches, s_e_scores))
                ...
                #             fps, args, patches))
                    ...
                    for ws in ws_starts:
                        ...
                        # length = []
                        # while len(length) < self.num_patches: # 10 is num_patch
                        #     # start = random.randint(locations[0], locations[-1] - 1)
                        #     # end = random.randint(start, locations[-1] - 1)
                        #     start = random.choice(gt_frame)
                        #     end = random.choice(gt_frame)
                        #     if end == start:
                        #         continue
                        ...
                        #     T_start =  int(start // 8 - 7)
                        #     T_end =  int(end // 8 - 7 )
                        #     if end > num_frames or T_start < 0 or T_end > vid_feature.shape[0] or (T_end - T_start) < 2:
                        #         continue
                        #     segment_frame = [start, end]
                        #     # print(num_frames)
                        #     # print(segment_frame)
    ...
    def get_data(self, video: VideoRecord):
        ...
        return vid,  snippet_fts, targets, patches, locations, s_e_scores
