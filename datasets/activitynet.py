...
class VideoRecord:
    def __init__(self, vid, num_frames, gt, fps, args, patches):
        self.id = vid
...
class ActivityDetection(torch.utils.data.Dataset):
    def __init__(self, feature_folder, tem_folder, anno_file, split, args):
        annotations = load_json(anno_file)['database']
        video_list = annotations.keys()
        self.window_size = args.window_size
        self.feature_folder = feature_folder
        self.tem_folder = tem_folder
        self.anno_file = load_json(anno_file)['database']
        ...
        for vid in video_list:
            # if self.split in vid:
                ...
                filename = os.path.join(self.feature_folder, 'v_' + vid +'.npy')
