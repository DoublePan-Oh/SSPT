...
class Ucf101Detection(torch.utils.data.Dataset):
    def __init__(self, feature_folder, tem_folder, anno_file, split, args):
        ...
        for i, vid in enumerate(video_list):
              ...
              #   while len(length) < self.num_patches: # 10 is num_patch
                while len(length) < 1: # 10 is num_patch
              ...
              #       T_end = int(math.ceil(end_iloc // 32) )
                    T_end = T_start + (T_random_end - T_random_start)
                    if T_start== T_end or T_end > vid_feature.shape[0] :
                           continue
                    ...
                    # patch = patch.permute(1,0)
                    patch_ft = []
                    for i in range(T_random_start , T_random_end):
                    ...
                    print("patch:",T_random_start, T_random_end)
                    print("vid:",T_start, T_end)

              #       temp = torch.zeros(vid_feature.shape)
