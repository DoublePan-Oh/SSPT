...
class VideoRecord:
    def __init__(self, vid, num_frames, gt, fps, args, patches, negtive):
        self.id = vid
...
class ThumosDetection(torch.utils.data.Dataset):
        ...
        for vid in video_list:
            # if self.split in vid:
                ...
                while len(length) < self.num_patches: # 10 is num_patch
                    ...
                    # delt_d = random.randint(4, num_frames-1)
                    
                    negtive_start = start + 8
                    negtive_end = end + 8

                    # delt_d = random.randint(4, num_frames-1)
                    
                    # negtive_start = end - 8
                    # negtive_end = 2 * end - start -8 #  negtive_start + end - start
                    # negtive_start = end
                    # negtive_end = negtive_start + pesudo_num_length
                    ...
                    T_negstart =  int(negtive_start // 8 - 7)
                    T_negend =  int(negtive_end // 8 - 7 )

                    # T_start =  int((4 / fps) * (start / fps))
                    # T_end =  int((4 / fps) * (end / fps))
                    if end > num_frames or T_start < 0 or T_end > vid_feature.shape[0] or (T_end - T_start) < 2 or T_negend > vid_feature.shape[0] or negtive_start > num_frames or negtive_end > num_frames or (T_end - T_start) != (T_negend-T_negstart) :
                        continue
                    ...
                    indice_neg = torch.tensor([m for m in range(T_negstart , T_negend )])
                    neg = torch.index_select(vid_feature, 0, indice_neg)
                    neg = torch.tensor(resizeFeature(neg,20),dtype=torch.float32).squeeze_(1).squeeze_(1)
                    negtive.append(neg)
    ...
    def get_data(self, video: VideoRecord):
        ...
        negtive_feature = video.negtive_feature
        ...
        negtive_feature = torch.stack(negtive_feature)
        ...
        return vid,  snippet_fts, targets, patches, negtive_feature
