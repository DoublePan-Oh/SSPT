...
class ThumosDetection(torch.utils.data.Dataset):
    def __init__(self, feature_folder, tem_folder, anno_file, split, args):
        ...
        self.window_size = args.window_size # 100
        ...
        for vid in video_list:
            if self.split in vid:
                ...
                annotations =[]
                labels = []
                while len(length) < 10: # 10 is num_patch
                    start = random.randint(0, num_frames)
                    end = random.randint(start, num_frames)
    ...
    def get_data(self, video: VideoRecord):
        ...
        return snippet_fts, targets, s_e_scores
