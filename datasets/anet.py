...
class VideoRecord:
    def __init__(self, vid, num_seconds, num_frames, gt, args,patches):
        self.id = vid
...
class ActivityDetection(torch.utils.data.Dataset):
    def __init__(self, feature_folder, tem_folder, anno_file, split, args):
        ...
        for vid in video_list:
            ...
            # if self.split in vid:
              ...
              #   duration = self.anno_file[vid]['duration']
                fps = 25
              #   num_frames = int(duration * fps)
              #   print(vid)
              #   print(self.anno_file[vid])
              #   print(self.anno_file[vid]["duration_second"])
                num_seconds = float(self.anno_file[vid]["duration_second"])
              #   print(num_seconds,"#######")
                num_frames = int(self.anno_file[vid]["duration_frame"])
                num_frames = num_frames // 16 * 16
                num_seconds = float(num_frames) / num_frames * num_seconds
                ...
                filename = os.path.join(self.feature_folder,  vid)
                vid_feature = torch.load(filename)
              #   vid_feature = torch.tensor(feats)
                ...
                while len(length) < self.num_patches: # 10 is num_patch
                    start = random.uniform(0, num_seconds )
                    end = random.uniform(start, num_seconds)

                    # start = random.uniform(0, duration)
                    # end = random.uniform(start,duration)
                    # pesudo_num_length = 160
                    pesudo_num_length = end - start
                    # end = start + pesudo_num_length 
                    # T_start =  int(start // 8 - 7)
                    # T_end =  int(end // 8 - 7 )
              #       T_start =  int((16 / fps) * (start / fps))
              #       T_end =  int((16 / fps) * (end / fps))
                    T_start =  int(start * fps // 16)
                    T_end =  int(end * fps // 16)
                    if end > num_seconds or T_start < 0 or T_end > vid_feature.shape[0] or T_start == T_end:
                        continue
              ...
              #           for idx in range(len(annotations))]
                gt = []
                for idx in range(len(annotations)):
                     gt.append((annotations[idx], labels[idx]))
              #   print(gt)
                self.video_list.append(
                     VideoRecord(vid, num_seconds, num_frames, gt,
                            args, patches))
    ...
    def get_data(self, video: VideoRecord):
       ...
       #  vid_feature = torch.tensor(feats)
        ...
        return vid,  snippet_fts, targets, patches
...
def build(split, args):
    ...
    assert root.exists(
    ), f'provided activityNet feature path {root} does not exist'
    feature_folder = root
    tem_folder = Path(args.tem_path)
    anno_file = Path(args.annotation_path)

    dataset = ActivityDetection(feature_folder, tem_folder, anno_file, split,
                              args)
    return dataset
