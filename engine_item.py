...
def train_one_epoch(model: torch.nn.Module,
    ...
    for vid_name_list, samples, targets, patches, negtive_feature \
        in metric_logger.log_every(data_loader, print_freq, header): 
    # for vid_name_list, samples, locations, targets, num_frames,  s_e_scores, patches, base \
    #     in metric_logger.log_every(data_loader, print_freq, header):
    # for vid_name_list, samples, targets, patches, locations, s_e_scores,  \
    #     in metric_logger.log_every(data_loader, print_freq, header):    
        samples = samples.to(device)
        # s_e_scores = s_e_scores.to(device)
        targets = [{k: v.to(device) for k, v in t.items()} for t in targets]
        patches = patches.to(device)
        negtive_feature = negtive_feature.to(device)

        outputs = model(samples, patches, negtive_feature)
        # print(targets)
        print("~~~~~~~~~~~~~~")
        # print(outputs)
        # exit()
